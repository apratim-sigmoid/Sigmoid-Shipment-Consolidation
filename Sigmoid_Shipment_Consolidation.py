...
logo = Image.open("sigmoid-logo.png")
...
def create_utilization_chart(all_consolidated_shipments):
    ...
    fig = go.Figure(data=[go.Bar(x=list(utilization_distribution.keys()), y=list(utilization_distribution.values()), marker_color='#f17e7b')])
    fig.update_layout(
...
def create_metric_box(label, value, color_start="#ee7276", color_end="#d86a67"):
    html = f"""
    <div style="
        ...
        with st.spinner("Running simulation..."):
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    mode='lines+markers',
                    line=dict(color='#4265c0', width=2),
                    marker=dict(size=8),
...
with tab2:
    ...
    if st.button("Calculate"):
        ...
        with st.spinner("Calculating..."):
            ...
            utilization_data = [
                (f'High utilization (>65%, >{high_utilization_pallets} pallets)', high_utilization, high_utilization/total_shipments*100, '#d86a67'),  # Dark red
                (f'Medium utilization (25-65%, {medium_utilization_pallets_low}-{medium_utilization_pallets_high} pallets)', medium_utilization, medium_utilization/total_shipments*100, '#f17e7b'),  # Medium red
                (f'Low utilization (<=25%, <={medium_utilization_pallets_low} pallets)', low_utilization, low_utilization/total_shipments*100, '#f9a3a1')  # Light red
            ]
            ...
            def display_metric(title, value):
                st.markdown(f"""
                    <div style="
                        ...
                        overflow: hidden;
                    ">
                        <p style="font-size: 16px; font-weight: 600; margin: 0 0 0px 0; color: #E30B14; position: relative; z-index: 1;">{title}</p>
                        <p style="font-size: 18px; font-weight: 600; margin: 0; color: #E30B14; position: relative; z-index: 1;">{value}</p>
                        <div style="
                            position: absolute;
                            top: 0;
                            left: 0;
...
fig_pallets = px.histogram(
    ...
    x="Total Pallets", 
    nbins=50, 
    color_discrete_sequence=['#f17e7b']
)
...
fig_pallets.update_traces(marker_color='#f17e7b')  # You can change this color as needed

# Display the chart without using container width
st.plotly_chart(fig_pallets)

st.sidebar.info('This dashboard provides insights into shipment consolidation. Use tabs to switch between simulation and calculation modes.')
